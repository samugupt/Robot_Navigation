...
def main():
    global area, dest_position, state, theta, theta_tolerance, x, y,i, goal_no_
    ...
    sub_goal = rospy.Subscriber("/goals", PointArray, clbk_goalpoints)
	...
	if math.sqrt(pow(dest_position.y - y, 2) + pow(dest_position.x - x, 2)) <= 0.3:
					...
					i=i+1
					goal_no_ += 1
					rospy.loginfo("[%d] Goal reached!" %goal_no_)
